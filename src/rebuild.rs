...
pub fn handle(args: RebuildArgs) -> Result<()> {
    ...
    let store_id = header.get("store_id").and_then(|v| v.as_str()).context("Missing store_id")?.to_string();
    let expected_first_sequence = header.get("first_sequence").and_then(|v| v.as_u64()).context("Missing first_sequence")?;
    let expected_through_sequence = header.get("through_sequence").and_then(|v| v.as_u64()).context("Missing through_sequence")?;
    let expected_predecessor_hash = header.get("previous_checkpoint_hash").and_then(|v| v.as_str()).context("Missing previous_checkpoint_hash")?.to_string();
    let expected_head_hash = header.get("head_record_hash").and_then(|v| v.as_str()).context("Missing head_record_hash")?.to_string();
    let expected_count = header.get("record_count").and_then(|v| v.as_u64()).context("Missing record_count")?;
    ...
    store.conn.execute("UPDATE store_metadata SET store_id = ?1", rusqlite::params![store_id])?;
    ...
    for line_result in lines {
        ...
        if envelope.get("record_schema_version").and_then(|v| v.as_i64()) != Some(1) {
            anyhow::bail!("Unsupported record schema version");
        }
        
        let local_sequence = envelope.get("local_sequence").and_then(|v| v.as_u64()).context("Missing local_sequence")?;
        let record_id = envelope.get("record_id").and_then(|v| v.as_str()).context("Missing record_id")?.to_string();
        let record_type = envelope.get("record_type").and_then(|v| v.as_str()).context("Missing record_type")?.to_string();
        let entity_id = envelope.get("entity_id").and_then(|v| v.as_str()).context("Missing entity_id")?.to_string();
        let captured_at = envelope.get("captured_at").and_then(|v| v.as_str()).context("Missing captured_at")?.to_string();
        let previous_record_hash = envelope.get("previous_record_hash").and_then(|v| v.as_str()).context("Missing previous_record_hash")?.to_string();
        let envelope_hash = envelope.get("record_hash").and_then(|v| v.as_str()).context("Missing record_hash")?.to_string();
        let payload_val = envelope.get("canonical_payload").context("Missing canonical_payload")?;
        ...
        if local_sequence != current_expected_sequence {
            anyhow::bail!("Non-contiguous sequence: expected {}, got {}", current_expected_sequence, local_sequence);
        }
        
        // 6. Verify predecessor hash
        if previous_record_hash != current_previous_hash {
            anyhow::bail!("Predecessor hash mismatch at sequence {}: expected {}, got {}", local_sequence, current_previous_hash, previous_record_hash);
        }
    ...
    if count != expected_count {
        anyhow::bail!("Record count mismatch: expected {}, got {}", expected_count, count);
    }
